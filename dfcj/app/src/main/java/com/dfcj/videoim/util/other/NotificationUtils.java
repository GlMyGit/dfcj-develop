...
import com.dfcj.videoim.SplashActivity;
    ...
    public  Notification getChannelNotificationQ(String title, String content, String type) {
        ...
        NotificationCompat.Builder notificationBuilder =
                new NotificationCompat.Builder(this, id)
                        .setSmallIcon(R.mipmap.my_log1)
                        //.setContentTitle(title)
                        //.setTicker(content)
                       // .setContentText(content)
                        .setOngoing(true) //ture，设置他为一个正在进行的通知。他们通常是用来表示一个后台任务,用户积极参与(如播放音乐)或以某种方式正在等待,因此占用设备(如一个文件下载,同步操作,主动网络连接)
                        .setAutoCancel(false)//设置这个标志当用户单击面板就可以让通知将自动取消
                     //   .setCustomContentView(new RemoteViews(MyAppliaction.getAppContext().getPackageName(), R.layout.view_notify))
       ...
       // incomingCallNotification.flags|= Notification.FLAG_ONGOING_EVENT;
        incomingCallNotification.flags= Notification.FLAG_ONGOING_EVENT;
        incomingCallNotification.flags= Notification.FLAG_NO_CLEAR;


      /*  Intent resultIntent = new Intent(this, WidgetAlertActivity.class);
        TaskStackBuilder stackBuilder = TaskStackBuilder.create(this);
// 添加返回栈
        stackBuilder.addParentStack(WidgetAlertActivity.class);
// 添加Intent到栈顶
