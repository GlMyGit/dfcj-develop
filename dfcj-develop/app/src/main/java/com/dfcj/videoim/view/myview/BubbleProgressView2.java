...
public class BubbleProgressView2 extends View {
    ...
    private int mColorProgressStr = getResources().getColor(R.color.color_FFE96F);//进度条的进度文字的颜色
    private int mColorProgressStr2 = getResources().getColor(R.color.color_FFE5DF);//进度条的进度文字的颜色
    private float mProgressHeight =   AppUtils.px2dip(getContext(),60);
    private float mProgress=0;//进度条的进度
    private float mBubbleTriangleHeight = 1;//气泡底部小三角高度
    private float mBubbleRectRound = AppUtils.px2dip(getContext(),1);//气泡的圆角
    private String mProgressStr = "再抽100次100%中奖";//显示进度的字符串
    private float mTextSize = AppUtils.sp2px(getContext(),1);//进度条文字大小
    private Paint.FontMetricsInt mFontMetricsInt;
    private float mProgressStrMargin = AppUtils.px2dip(getContext(),1);//气泡的边距
    public BubbleProgressView2(Context context) {
        this(context,null);
    }

    public BubbleProgressView2(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs,0);
    }

    public BubbleProgressView2(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        ...
        mColorProgressBg = getResources().getColor(R.color.color_DEDEDE);
        mColorProgress = getResources().getColor(R.color.color_FFBF2E);
        mPaintBubble.setColor(getResources().getColor(R.color.color_DD4349));//设置气泡的颜色
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mPathSrc.moveTo(30,h/2);
        mPathSrc.lineTo(w-15,h/2);//进度条位置在控件整体底部，且距离控件左边和右边各20像素
    ...
    private void drawBubble(Canvas canvas) {
        Rect rect = new Rect();
        mPaintProgressStr.getTextBounds(mProgressStr,0,mProgressStr.length(),rect);
        ...
        RectF rectF = new RectF(p[0]-mBubbleTriangleHeight-mBubbleRectRound/2-(mProgress*width),
    ...
    public void setProgress(float progress){
        mProgress = progress;
        //mProgressStr = "还差"+(progress/100)+"提现";;
        invalidate();//设置完进度进行重绘
    }


    public void setProVal(float progress,float mycha,int maxpro){
        //LogUtils.logd("当前:"+progress+"   差多少："+mycha+"   总的："+maxpro);
      //  LogUtils.logd("   差多少："+(mycha*100));
        mProgress = progress;
        mProgressStr = "再抽"+(mycha)+"次100%中奖";
        invalidate();//设置完进度进行重绘
    }
    ...
    public void setProgressWithAnim(float progress){
        ObjectAnimator.ofFloat(this,"progress",0,progress).setDuration(3000).start();
    }

    public void setProgressWithAnim(float statuVal,float progress){
        ObjectAnimator.ofFloat(this,"progress",statuVal,progress).setDuration(3000).start();
    }
}
